...
def tile_info_and_weighted_sum_gen(model, model_name, dataset_name, max_power):
    ...
    h_lvl_w = (n_lvl_w - 2) / 2
    input = torch.load(f"trace_data/inputs_{model_name}_synthetic.pt")
    on_off_ratio = 17
    K = torch.tensor([1/on_off_ratio,(1/3+2/(3*on_off_ratio)),(2/3+1/(3*on_off_ratio)),1])
    idx=0
    for  (name, layer) in (model.named_modules()):
        if isinstance(layer, nn.Linear):
            ...
            # print("crxb_row_pads",crxb_row_pads)
            ...
            input_layer = input_layer.unsqueeze(0)
    ...
    torch.save(power_trace, f"trace_data/power_trace_{model_name}_synthetic.pt")
...
if  __name__ == "__main__":
    ...
    for model_name in model_names:
        ...
        tile_info_and_weighted_sum_gen(model, model_name, dataset_name, max_power)