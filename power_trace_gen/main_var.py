...
from VGG import *
from create_floorplan_variable import *
    ...
    for model_name in model_names:
        ...
        for attack_name in attack_names:
            for config in configs:
                ...
                path = original_directory + f'/results_var/{attack_name}/{model_name}/{config*100}_{(1-config)*100}'
...
'''
from Hotspot Simulations extract the new temperature of each tile for accuracy framework 
'''
for mode in ['attacker', 'normal']:
    for model_name in model_names:
...
'''
Run the accuracy framework the see the impact of the new eleveated temperature due to the attacks.  
'''
...
Parsing arguments:
 parser = argparse.ArgumentParser(description='PyTorch CIFAR10 Training')
    ...
    for this accuracy simulation args.tm and args thermal_mode and args.enable_fec will remain at their edefault values. 

'''
accuracy_path = original_directory + '/../topk_inference/benchmark'
os.chdir(accuracy_path)
for mode in ['attacker']:
    for model_name in model_names:
        for attack_name in attack_names:
            for config in configs:
                ...
                command = [
                    ...
                    '--cuda', 'cuda:0',
                    '--fault_dist', 'uniform',
                    '--path', f'{original_directory}/results_var/{attack_name}/{model_name}/{config*100}_{(1-config)*100}/new_tile_temp_{mode}.pt'
                ]
                output_file_path = f"{original_directory}/results_var/{attack_name}/{model_name}/{config*100}_{(1-config)*100}/accuracy_output_{mode}.txt"
