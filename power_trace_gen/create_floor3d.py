...
def gen_floor_plan_3d_and_power_trace(model_name, dataset_name, max_power_array,peripheral_power,mode):
    ...
    for attack in attack_type:
        os.makedirs(f"./results3d/{attack}", exist_ok=True)
        attack_path = f"./results3d/{attack}"
        for tile in tile_config:
            os.makedirs(f"./results3d/{attack}/{model_name}/{tile}", exist_ok=True)
            tile_path = attack_path + f"/{model_name}/{tile}"
            for flp in flp_config:
                os.makedirs(f"./results3d/{attack}/{model_name}/{tile}/{flp}", exist_ok=True)
    ...
    for attack in attack_type:
        ...
        for tile in tile_config:
            ...
            for flp in flp_config:
                ...
                gen_input_weights_traces(attack, model_name, dataset_name, tiles_for_attacker, max_power_array, path,mode)
    ...
    for attack in attack_type:
        for tile in tile_config:
            if tile == "50_50":
                ...
                Ny = math.ceil(tile_per_tier/Nx)
            for flp in flp_config:
                path = f"./results3d/{attack}/{model_name}/{tile}/{flp}"
                load_attack_power_info_1 = torch.load(f"{path}/power_trace_attck_{model_name}_{dataset_name}_{mode}.pt")
...
def main():
    ...
    for mode in ['attacker', 'normal']:
        for model_name in model_names:
            print(f"Generating power traces for {model_name} model in {mode} mode...")
            gen_floor_plan_3d_and_power_trace(model_name, "cifar10", max_power_array,peripheral_power,mode)
