...
def gen_floor_plan_2d_and_power_trace(model_name, dataset_name, max_power_array,peripheral_power, mode):
    ...
    attack_type = ["weight_input_based_attack"]
    # tile_configs = ["50_50", "20_80", "80_20", "special"]
    tile_configs = ["50_50"]
    ...
    for attack in attack_type:
        for tile_config in tile_configs:
            attack_path = original_directory+f"/results/{attack}"
            os.makedirs(attack_path, exist_ok=True)
            model_path = f"{attack_path}/{model_name}"
            os.makedirs(model_path, exist_ok=True)
            tile_config_path = f"{model_path}/{tile_config}"
            ...
            if tile_config == "50_50":
                total_tiles = 2*user_tiles
                Nx = math.ceil(math.sqrt((total_tiles)))
                Ny = math.ceil(total_tiles/Nx)
                dummy_tiles = Nx*Ny - total_tiles ## All dummy tiles can be used by attacker so no need to explictly add them and have them in the floorplan.
                tiles_for_attacker = total_tiles - user_tiles + dummy_tiles
            elif tile_config == "20_80":
                total_tiles = math.ceil(user_tiles/0.2)
                Nx = math.ceil(math.sqrt((total_tiles)))
                Ny = math.ceil(total_tiles/Nx)
                dummy_tiles = Nx*Ny - total_tiles
                tiles_for_attacker = total_tiles - user_tiles + dummy_tiles
            elif tile_config == "80_20":
                total_tiles = math.ceil(user_tiles/0.8)
                Nx = math.ceil(math.sqrt((total_tiles)))
                Ny = math.ceil(total_tiles/Nx)
                dummy_tiles = Nx*Ny - total_tiles
                tiles_for_attacker = total_tiles - user_tiles + dummy_tiles
            elif tile_config == "special":
                Nx = math.ceil(math.sqrt((user_tiles))) + 2
                Ny = math.ceil(user_tiles/Nx) + 2
                total_tiles = Nx*Ny
            ...
            for i in range(len(load_attack_power_info)):
                print("length of the power info",len(load_attack_power_info[i]))
                print("length of the power info",sum(load_attack_power_info[i]))
