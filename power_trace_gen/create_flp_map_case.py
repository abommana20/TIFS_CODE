...
def gen_floor_plan_2d_and_power_trace(model_name, dataset_name, max_power_array,peripheral_power):
    ...
    attack_path_1 = f"./results_map/input_based_attack"
    attack_path_2 = f"./results_map/weight_based_attack"
    attack_path_3 = f"./results_map/weight_input_based_attack"

    ##create folders for the attack
    os.makedirs(attack_path_1, exist_ok=True)
    os.makedirs(attack_path_2, exist_ok=True)
    os.makedirs(attack_path_3, exist_ok=True)

    model_path_1 = f"{attack_path_1}/{model_name}"
    model_path_2 = f"{attack_path_2}/{model_name}"
    model_path_3 = f"{attack_path_3}/{model_name}"

    ##create folders for the model
    os.makedirs(model_path_1, exist_ok=True)
    os.makedirs(model_path_2, exist_ok=True)
    os.makedirs(model_path_3, exist_ok=True)
    ...
    floorplan_path_1_attack_2= f"{model_path_2}/50_50"

    os.makedirs(floorplan_path_1_attack_2, exist_ok=True)

    floorplan_path_1_attack_3 = f"{model_path_3}/50_50"

    os.makedirs(floorplan_path_1_attack_3, exist_ok=True)
    ...
    # tiles_for_attacker_50_50 = total_tile_50_50 - total_tile + dummy_tiles_50_50
    tiles_for_attacker_50_50 = total_tile
    remaining_tiles = Nx_50_50*Ny_50_50 - 2*total_tile

    print("total_tile_50_50",Nx_50_50*Ny_50_50)
    print("attack_tiles_50_50",tiles_for_attacker_50_50)
    print("user_tiles_50_50",total_tile)
    generate_2d_floorplan(Nx_50_50, Ny_50_50, tx, ty, unit_name_tile, floorplan_path_1_attack_1)
    generate_2d_floorplan(Nx_50_50, Ny_50_50, tx, ty, unit_name_tile, floorplan_path_1_attack_2)
    generate_2d_floorplan(Nx_50_50, Ny_50_50, tx, ty, unit_name_tile, floorplan_path_1_attack_3)

    gen_input_weights_traces("input_based_attack", model_name, dataset_name, tiles_for_attacker_50_50, max_power_array, floorplan_path_1_attack_1)
    gen_input_weights_traces("weight_based_attack", model_name, dataset_name, tiles_for_attacker_50_50, max_power_array, floorplan_path_1_attack_2)
    gen_input_weights_traces("weight_input_based_attack", model_name, dataset_name, tiles_for_attacker_50_50, max_power_array, floorplan_path_1_attack_3)
  ...
  ##################################################################
    ...
    torch.save(user_power_trace, f"./results/final_power_trace_{model_name}_{dataset_name}")

    load_attack_power_info_1 = torch.load(f"{floorplan_path_1_attack_1}/power_trace_attck_{model_name}_{dataset_name}.pt")
    
    attac_power_trace_1 = user_power_trace.copy()
    
    ## apend two list till the special case
    combined_trace_1 = user_power_trace + attac_power_trace_1 + [0]*remaining_tiles
    
    ##savethem in thirs respective folders
    torch.save(combined_trace_1, f"{floorplan_path_1_attack_1}/combined_power_trace_{model_name}_{dataset_name}.pt")

    ## create the power trace file from the lists. 
    output_trace_file_1= f"{floorplan_path_1_attack_1}/power_trace_{model_name}_{dataset_name}.ptrace"

    create_ptrace_file(combined_trace_1, output_trace_file_1)
##################################################################
    ##repeat for the other two attacks
    load_attack_power_info_1 = torch.load(f"{floorplan_path_1_attack_2}/power_trace_attck_{model_name}_{dataset_name}.pt")
    ...
    for i in range(len(load_attack_power_info_1)):
        attac_power_trace_1.append(sum(load_attack_power_info_1[i])+ len(load_attack_power_info_1[i])*peripheral_power)
    
    ## apend two list till the special case
    combined_trace_1 = user_power_trace + attac_power_trace_1
    # print("len of user tiles",len(user_power_trace))
    ##savethem in thirs respective folders
    torch.save(combined_trace_1, f"{floorplan_path_1_attack_2}/combined_power_trace_{model_name}_{dataset_name}.pt")
    ##create ptrace
    output_trace_file_1= f"{floorplan_path_1_attack_2}/power_trace_{model_name}_{dataset_name}.ptrace"

    create_ptrace_file(combined_trace_1, output_trace_file_1)

##################################################################
    ##for attack 3
    load_attack_power_info_1 = torch.load(f"{floorplan_path_1_attack_3}/power_trace_attck_{model_name}_{dataset_name}.pt")
    ...
    ##savethem in thirs respective folders
    torch.save(combined_trace_1, f"{floorplan_path_1_attack_3}/combined_power_trace_{model_name}_{dataset_name}.pt")

    ##create ptrace
    output_trace_file_1= f"{floorplan_path_1_attack_3}/power_trace_{model_name}_{dataset_name}.ptrace"
...
def main():
    max_power_array = 0.3
    peripheralpower=2.7
    gen_floor_plan_2d_and_power_trace("resnet18", "cifar10", max_power_array,peripheralpower )
