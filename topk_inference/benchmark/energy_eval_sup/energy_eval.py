import torch
import numpy as np
def baseline_eval (model_name,
    dataset,
    tile_info_path="../tile_info",
    latency_path="../latency",
    power_trace_path = "../power_trace", # Path to the already-saved baseline energy dict
    speed_up_path = "./",
    speed_up_fac = 1):
    energy_model_dict = {}
    ...
    speed_up = torch.load(f"{speed_up_path}/speed_up_{model_name}.pt", weights_only = True)
    ...
    latency_list = []
    for idx, key in enumerate(latency_info.keys()):
        latency_list.append(latency_info[key]/speed_up[idx])
    tile_end_idx = 0
    energy_list = []
    ...
    for layer in range(len(tile_info)):
        ...
        power_cummulative = sum(power_trace_model[tile_start_idx:tile_end_idx])*speed_up[layer]
    ...
    torch.save(energy_model_dict,f"./energy_eval_sup/baseline_energy_spup_{speed_up_fac}.pt" ) ##nJ
def energy_eval(
    ...
    latency_path="../latency",
    baseline_energy_path="baseline_energy_spup_1.pt",  # Path to the already-saved baseline energy dict
    speed_up_path = "./",
    max_storage=0.12,
    error_bits=4
):
    ...
    for layer in range(len(tile_info)):
        ...
        pec_layer = crosbar_list[layer] * power_ec + power_cummulative_dram[layer]*speed_up[layer]
